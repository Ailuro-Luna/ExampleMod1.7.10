...
def generate_java_code(grouped_blocks):
    ...
    for mod_id, blocks in grouped_blocks.items():
        ...
        for block in blocks:
            ...
            block_str = f'    {{"{block["block_name"]}", {block["id"]}, "rock", {texture_str}}}'
            blocks_array.append(block_str)
        
        # 构建完整的数组定义 - 这里注意处理Java代码中的大括号
        mod_variable_name = mod_id.lower()
        full_variable_name = mod_variable_name + "BlockData"
        mod_array_code = f'''Object[][] {full_variable_name} = {{
{",\n".join(blocks_array)}
}};
BlockRegistryHelper.registerBlocks("{mod_id}", {full_variable_name});
'''
...
def main():
    # 设置输入和输出文件路径
    log_file_path = "minecraft_log.log"
